...
export default class InteractionEvents {
    init(rdelay) {
        ...
        if (rdelay < 10000) { // max 10 seconds
            dispatcher.on('i', () => {
                // sometimes first interaction happens before images are loaded, 
                // that means there is no need to set this timeout
                if (!firstInteractionFired) {
                    process.env.DEBUG && c(delta(), separator, "installing firstInteraction timeout for " + rdelay + "ms");
                    firstInteractionTimeout = setTimeout(onFirstInteraction, rdelay);
                }
            });
        }
