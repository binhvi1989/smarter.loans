...
$VULNERABLE_PLUGINS = array(
    ...
    'file_folder_manager.php' => array('5.4', 'https://labs.sucuri.net/plugins-under-attack-june-2019/', 'File Manager'),
    ...
    'brizy.php' => array('1.0.114', 'https://wpvulndb.com/vulnerabilities/10112', 'Brizy – Page Builder'),
    ...
    'ninja-forms.php' => array('3.5.0', 'https://wordpress.org/plugins/ninja-forms/#developers', 'Ninja Forms - Security Issues 3.4.34.1'),
    ...
    'cleantalk.php' => array('5.153.4', 'https://wpscan.com/vulnerability/152171fc-888c-4275-a118-5a1e664ef28b', 'Anti-Spam by CleanTalk'),
    ...
    'woocommerce.php' => array('5.5.1', 'https://wpscan.com/vulnerability/1212fec8-1fde-41e5-af70-abdd7ffe5379', 'WooCommerce'),
    'download-manager.php' => array('3.1.25', 'https://wpscan.com/vulnerability/0f2089dc-9376-4d7d-95a2-25c99526804a', 'WordPress Download Manager'),
    'seopress.php' => array('5.0.4', 'https://nvd.nist.gov/vuln/detail/CVE-2021-34641', 'SEOPress'),
    'download-manager.php' => array('3.1.25', 'https://wpscan.com/vulnerability/066947e2-cee1-4ae0-9158-a5750f2ac554', 'WordPress Download Manager'),
    ...
    'essential_adons_elementor.php' => array('5.0.5', 'https://patchstack.com/articles/critical-vulnerability-fixed-in-essential-addons-for-elementor-plugin/', 'Essential Addons for Elementor'),
    ...
    if (strpos($path, 'dev/tests/integration/framework/Magento/TestFramework/Event') != false) {
        ...
        if (strpos($path, 'vendor/magento/magento2-base') != false) {
            if (file_exists($path.'composer.json')) {
                ...
                return array('name' => 'Magento 2', 'version' => $version, 'dir' => $realdir, 'source' => $originpath.'/'.$fileName,
                'updated' => checkIsUpdated($version, $VERSIONS['magento']), );
            }
        } else {
            return;
...
function checkFreemiusVersion($fileName, $path)
{
    ...
    if (version_compare($version, '2.2.3') <= 0) {
        return array('name' => 'Freemius Library', 'version' => $version, 'dir' => $path.'/'.$fileName, 'oldplugin' => true, 'url' => 'https://wpvulndb.com/vulnerabilities/9223', );
    }
...
function checkElementorVersion($fileName, $path)
{
    ...
    if (version_compare($version, '2.9.7') <= 0) {
        return array('name' => 'Elementor', 'version' => $version, 'dir' => $path.'/', 'updated' => false, 'url' => 'https://wpvulndb.com/vulnerabilities/10213', );
    }
}
...
function checkUltimateMemberVersion($fileName, $path)
{
    ...
    if (version_compare($version, '2.1.11') <= 0) {
        return array('name' => 'Ultimate Member', 'version' => $version, 'dir' => $path.'/', 'updated' => false, 'url' => 'https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2020-36155', );
    }
}
 ...
 * @param   $dir    String - Directory to check for a WordPress installation on
 * @return  $res    Array - Containing message data to display
 *
 */
function checkPluginVersions($dir)
{
    include_once($dir . 'wp-load.php');
    include_once($dir . 'wp-admin/includes/misc.php');
    include_once($dir . 'wp-admin/includes/file.php');
	...
	include_once $dir . 'wp-admin/includes/class-wp-upgrader.php';
	wp_cache_flush();
	...
	foreach($updates_transient['response'] as $id => $result) {
		$version = $all_plugins[$updates_transient['response'][$id]['plugin']]['Version'];
                ...
                if (!isset($updates_transient['response'][$id]['upgrade_notice'])){
                    $updates_transient['response'][$id]['upgrade_notice'] = '';
                }
		
		$res = array('name' => $updates_transient['response'][$id]['slug'], 'version' => $version, 'dir' => $dir . 'wp-content/plugins/' . $updates_transient['response'][$id]['plugin'], 'updated' => false, 'url' => 'New Version: ' . $updates_transient['response'][$id]['new_version'] . ' ' . strip_tags($updates_transient['response'][$id]['upgrade_notice']) , );
